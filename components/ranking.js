...
return (
  <div className="ranking">
    ...
    <button onClick={goBackItems} disabled={currentIndex === 0}><span>{`<-`}</span></button>
    <button onClick={loadMoreItems} disabled={currentIndex + itemsPerPage >= highscores.length}><span>{`->`}</span></button>
  </div>
);
}
